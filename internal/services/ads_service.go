...
func NewAdsService(adsRepo *repo.AdsRepository, log *logger.Logger, kafka *KafkaService, cb *breaker.CircuitBreaker) *AdsService {
	return &AdsService{
		adsRepo:      adsRepo,
		log:          log,
		kafka:        kafka,
...
func (s *AdsService) PublishClick(click model.Clicks) error {
	...
	if err != nil {
		metrics.RecordError("marshal_click_error", "ads_service")
		return fmt.Errorf("failed to marshal click: %w", err)
	}

	message := &sarama.ProducerMessage{
		Topic: topicName,
		Key:   sarama.StringEncoder(click.AdID),
		Value: sarama.ByteEncoder(data),
	}

	partition, offset, err := s.kafka.producer.SendMessage(message)
	if err != nil {
		metrics.RecordError("kafka_publish_error", "ads_service")
		return fmt.Errorf("failed to publish click: %w", err)
	}

	s.log.Logger.Debugf("Click published to partition %d, offset %d", partition, offset)
	return nil
}
